...
`:extract`  | `[V|RGB|ARGB|RRGGBB|AARRGGBB] shading` | Extracts the translucent black-and-white shading of the specified reference color to a new image. Pixels with a similar hue become translucent white (lighter than the reference) or translucent black (darker), with alpha chosen so that overlaying the result on the reference color reproduces the original. Near-grayscale pixels are copied as true grayscale; all other pixels become transparent.
